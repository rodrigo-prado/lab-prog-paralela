...
int obtem_i_AC() {
  ...
  result = g_i_AC;
  if (result != parada_1) {
    if (g_i_AC == (num_candA - 1)) g_i_AC = 0;
    else g_i_AC += 1;
  }
...
int obtem_i_BC() {
  ...
  result = g_i_BC;
  if (result != parada_1) {
    if (g_i_BC == (num_candB - 1)) g_i_BC = 0;
    else g_i_BC += 1;
  }
  sem_post(&access_mutex);
  return result;
...
int obtem_i_AC1() {
  ...
  result = g_i_AC1;
  if (result != parada_1) {
    if (g_i_AC1 == (num_candA - 1)) g_i_AC1 = 0;
    else g_i_AC1 += 1;
  }
  sem_post(&access_mutex);
  return result;
...
void *processa_viz_down_A_v2(void *arg) {
  int vert_AC, i_AC;
  int DEPU = 0;
  bool moveu = false;
  ...
  // if (data->tid == 0) printf("%d:%d:Address of x is (dentro) %p\n", rank, data->tid, (void *) &data->cand2);
  // printf("hello from thr_func, thread id: %d\n", data->tid);
  /*if (data->cand2.getsize() > 0) {
    printf("\\o/\n");
    data->buga[0] = data->tid + 99;
  }*/
  ...
  while (i_AC != parada_1) {
    if (g_moveu) {
      // std::cout << rank << ":" << data->tid << ":A:Moveu!!!" << vert_AC << std::endl;
      break;
    }
    if (data->a[i_AC] != -2) {
      vert_AC = data->a[i_AC];
      ...
      data->C[vert_AC] = 1;
      data->a[i_AC] = -2;
      data->c[num_candC] = vert_AC;
      data->c[num_candC + 1] = -1;

      moveu = data->obj->move_2_C_AB_v2(vert_AC, 0, data->A, data->a, data->B, data->b,
          data->C, data->c, data->cand1, data->cand2, data->TEST);
      ...
      if (moveu == true) {
        ...
        if (!g_moveu) {
          g_moveu = true;
          g_tid = data->tid;
          // std::cout << "\x1b[1;38m" << rank << ":" << data->tid << ":" << "Uhuu!!!!" << "\x1b[0m" << std::endl;
        }
        sem_post(&access_mutex);
        // std::cout << std::endl << ":O = " << vert_AC << std::endl;
        break;
      } else {
        /* <--- */
        data->C[vert_AC] = 0;
...
void *processa_viz_down_B_v2(void *arg) {
  int vert_BC, i_BC;
  int DEPU = 0;
  bool moveu = false;
  ...
  while (i_BC != parada_1) {
    if (g_moveu) {
      // std::cout << rank << ":" << data->tid << ":B:Moveu!!!" << vert_BC << std::endl;
      break;
    }
    if (data->b[i_BC] != -2) {
      vert_BC = data->b[i_BC];
      ...
      data->C[vert_BC] = 1;
      data->b[i_BC] = -2;
      data->c[num_candC] = vert_BC;
      data->c[num_candC + 1] = -1;

      moveu = data->obj->move_2_C_AB_v2(vert_BC, 1, data->A, data->a, data->B, data->b,
          data->C, data->c, data->cand1, data->cand2, data->TEST);
      ...
      if (moveu == true) {
        ...
        sem_post(&access_mutex);
        // std::cout << std::endl << ":O = Depois de pthread_exit" << vert_BC << std::endl;
        break;
      } else {
        /* <--- */
        data->C[vert_BC] = 0;
...
void *processa_viz_2down_A_v2(void *arg) {
  ...
  //     parada_1 > 0; i_AC1++) {
  while (i_AC1 != parada_1) {
    if (g_moveu) {
      // std::cout << rank << ":" << data->tid << ":A:Moveu!!!" << vert_AC << std::endl;
      break;
    }
    if (data->a[i_AC1] != -2) {
      vert_AC1 = data->a[i_AC1];

      /* ---> */
      data->A[vert_AC1] = 0;
      data->C[vert_AC1] = 1;
      data->a[i_AC1] = -2;
      data->c[num_candC] = vert_AC1;
      data->c[num_candC+1] = -1;

      /* 1 NIVEL) 2A -> C */
      for (vert_AC2 = -1, parada_2 = num_candA, i_AC2 = (int) (rand() % num_candA);
          parada_2 > 0; i_AC2++) {
        if ((data->a[i_AC2] != -2) && (data->a[i_AC2] > vert_AC1)) {
          vert_AC2 = data->a[i_AC2];
          ...
          data->C[vert_AC2] = 1;
          data->a[i_AC2] = -2;
          data->c[num_candC+1] = vert_AC2;
          data->c[num_candC+2] = -1;

          moveu = data->obj->move_3_C_AB_v2(vert_AC1, vert_AC2, 0, 0, data->A,
              data->a, data->B, data->b, data->C, data->c, data->cand1,
              data->cand2, data->TEST);
...
void *processa_viz_2down_B_v2(void *arg) {
  ...
  i_BC1 = obtem_i_BC1();
  /* 1 NIVEL) 1B -> C */
  // for (vert_BC1 = -1, parada_1 = num_candA, i_BC1=(int)(rand() % num_candA);
  //     parada_1 > 0; i_BC1++) {

  /*for (vert_BC1 = -1, parada_1 = num_candB, i_BC1=(int)(rand() % num_candB);
      parada_1 > 0; i_BC1++) {*/
  while (i_BC1 != parada_1) {
    if (g_moveu) {
      // std::cout << rank << ":" << data->tid << ":A:Moveu!!!" << vert_BC << std::endl;
      break;
    }
    ...
    if (data->b[i_BC1] != -2) {
      ...
      data->C[vert_BC1] = 1;
      data->b[i_BC1] = -2;
      data->c[num_candC] = vert_BC1;
      data->c[num_candC + 1] = -1;

      /* 1 NIVEL) 2B -> C */
      for (vert_BC2 = -1, parada_2 = num_candB, i_BC2 = (int)(rand() % num_candB);
          parada_2 > 0; i_BC2++) {
        if ((data->b[i_BC2] != -2) && (data->b[i_BC2] > vert_BC1)) {
          vert_BC2 = data->b[i_BC2];
          if (DEPU) std::cout << std::endl << "vert_BB_C = " << vert_BC1 << ", "
              << vert_BC2 << std::endl;
          ...
          data->C[vert_BC2] = 1;
          data->b[i_BC2] = -2;
          data->c[num_candC+1] = vert_BC2;
          data->c[num_candC+2] = -1;

          moveu = data->obj->move_3_C_AB_v2(vert_BC1, vert_BC2, 1, 1, data->A, data->a,
              data->B, data->b, data->C, data->c, data->cand1, data->cand2,
              data->TEST);
    ...
    if (data->a[i_AC1] != -2) {
      ...
      for (vert_BC1 = -1, parada_2 = num_candB, i_BC1 = (int) (rand() % num_candB);
        ...
        if (data->b[i_BC1] != -2) {
          ...
          data->C[vert_BC1] = 1;
          data->b[i_BC1] = -2;
          data->c[num_candC+1] = vert_BC1;
          data->c[num_candC+2] = -1;

          moveu = data->obj->move_3_C_AB_v2(vert_AC1, vert_BC1, 0, 1, data->A, data->a,
              data->B, data->b, data->C, data->c, data->cand1, data->cand2,
              data->TEST);
    ...
    if ((vez == 1) && (!moveu) && (!testa_A)) {
      for (parada_2 = num_candC, i_swap = (int) (rand() % num_candC);
        ...
        if ((vert_swap != -2) && (stat[vert_swap] == 1) && (C[vert_swap] == 1)
            && ((desceu != vert_swap) || (origem != 0))) {
          e_cand = can_move_C_X_v2(vert_swap, a, b, C);

          if (e_cand) {
            if (DEPU) std::cout << std::endl<<"  vert_CA = "
              << vert_swap<<std::endl;
            A[vert_swap] = 1;
            C[vert_swap] = 0;
            c[i_swap] = -2;
            ...
            for (parada_3 = num_candC, l = (int) (rand() % num_candC);
              ...
              if (c[l] != -2) {
                ...
                if (j == 1) {
                  if (DEPU) std::cout << std::endl << "    vert_CA = " << c[l]
                    << " MOVEU !" << std::endl;
                  A[c[l]]  = 1; C[c[l]]  = 0;
                  atualiza_a_b_c(A, a, B, b, C, c);
                  moveu = true;
                ...
                if (j == 2) {
                  if (DEPU) std::cout << std::endl << "    vert_CB = " << c[l]
                    << " MOVEU !" << std::endl;
                  B[c[l]]  = 1; C[c[l]]  = 0;
                  atualiza_a_b_c(A, a, B, b, C, c);
                  moveu = true;
    ...
    if ((vez == 0) && (!moveu) && (!testa_B)) {
      for (parada_2=num_candC, i_swap=(int) (rand() % num_candC);
        ...
        if ((vert_swap != -2) && (stat[vert_swap] == 1) && (C[vert_swap] == 1)
          ...
          if (e_cand) {
            if (DEPU) std::cout << std::endl << "  vert_CB = " << vert_swap
              << std::endl;
            B[vert_swap] = 1;
            C[vert_swap] = 0;
            c[i_swap] = -2;
...
int Sigraph::viz_down_A_v2(Array<int> &A, Array<int> &a, Array<int> &B,
  ...
  //std::cout << rank << ":" << cand2[0] << ":" << thr_data[0].cand2[0] << std::endl;
  // if (meu_buga.getsize() > 0) std::cout << rank << ":" << meu_buga[0] << ":" << thr_data[0].buga[0] << std::endl;
  // printf("%d:0:Address of x is (fora) %p\n", rank, (void *) &thr_data[0].cand2);
  ...
  for (i = 0; i < concurentThreadsSupported; ++i) {
    ...
    thr_data[i].C = C;
    thr_data[i].c = c;
    thr_data[i].cand1 = cand1;
    thr_data[i].cand2 = cand2;
    thr_data[i].TEST = TEST;
    thr_data[i].t_ini_g = t_ini_g;
    thr_data[i].obj = this;
    if ((rc = pthread_create(&thr[i], NULL, processa_viz_down_A_v2, &thr_data[i]))) {
  ...
  if (g_moveu) {
    ...
    //std::cout << "\x1b[1;38m" << rank << ":" << "Ninguém moveu! :(" << "\x1b[0m" << std::endl;
  }
...
int Sigraph::viz_down_B_v2(Array<int> &A, Array<int> &a, Array<int> &B,
  ...
  for (i = 0; i < concurentThreadsSupported; ++i) {
    ...
    thr_data[i].t_ini_g = t_ini_g;
    thr_data[i].obj = this;
    if ((rc = pthread_create(&thr[i], NULL, processa_viz_down_B_v2, &thr_data[i]))) {
      fprintf(stderr, "error: pthread_create, rc: %d\n", rc);
      return EXIT_FAILURE;
    }
  }
...
int Sigraph::viz_2down_A_v2(Array<int> &A, Array<int> &a, Array<int> &B,
  ...
  for (i = 0; i < concurentThreadsSupported; ++i) {
    ...
    thr_data[i].t_ini_g = t_ini_g;
    thr_data[i].obj = this;
    if ((rc = pthread_create(&thr[i], NULL, processa_viz_2down_A_v2, &thr_data[i]))) {
      fprintf(stderr, "error: pthread_create, rc: %d\n", rc);
      return EXIT_FAILURE;
    }
  }
...
int Sigraph::viz_2down_B_v2(Array<int> &A, Array<int> &a, Array<int> &B,
  ...
  for (i = 0; i < concurentThreadsSupported; ++i) {
    ...
    thr_data[i].t_ini_g = t_ini_g;
    thr_data[i].obj = this;
    if ((rc = pthread_create(&thr[i], NULL, processa_viz_2down_B_v2, &thr_data[i]))) {
      fprintf(stderr, "error: pthread_create, rc: %d\n", rc);
      return EXIT_FAILURE;
    }
  }
...
int Sigraph::viz_2down_AB_v2(Array<int> &A, Array<int> &a, Array<int> &B,
  ...
  for (i = 0; i < concurentThreadsSupported; ++i) {
    ...
    thr_data[i].t_ini_g = t_ini_g;
    thr_data[i].obj = this;
    if ((rc = pthread_create(&thr[i], NULL, processa_viz_2down_AB_v2, &thr_data[i]))) {
      fprintf(stderr, "error: pthread_create, rc: %d\n", rc);
      return EXIT_FAILURE;
    }
  }
